				...
				<div class="layui-card">
					...
					<div class="layui-card-body ">
						<table class="layui-table layui-form">
							...
							<tbody>
								<form:form method='POST' modelAttribute="up_booking"
									class='form-horizontal' onsubmit="return testIf()">
									<tr>
										
										<td><form:input type="hidden" path="mb_id"/>${up_booking.mb_id}</td>
										<td>${up_booking.userInfoBean.u_account}</td>

										<td>
											<form:select path="userCarBean.uc_id">
												<form:options items="${CarList}"/>
											</form:select>
										</td>

										<td>
											<form:select path="maintainInfoBean.mi_id">
												<form:options items="${SetList}"/>
											</form:select>
										</td>
										
										<td>
											<form:select path="maintainStationBean.ms_id" id="MBstation">
...
<!--       </div> -->
										...
										<c:if test="${up_booking.mb_status==1}">
											<th>預約中</th>
										</c:if>
										<c:if test="${up_booking.mb_status==0}">
											<th>已取消</th>
										</c:if>
										<c:if test="${up_booking.mb_status==2}">
											<th>保養完成</th>
										</c:if>																			
										<td class="td-manage">
																		
											<a title="確認" style="margin-left: 20px;">
									...
									<form:input type="hidden" path="mb_createdate" />
									<form:input type="hidden" path="mb_modifieddate" />
									<form:input type="hidden" path="mb_status" />
