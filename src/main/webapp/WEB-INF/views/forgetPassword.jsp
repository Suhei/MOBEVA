...
<body>
	...
	<div class="overflow-hidden">
		...
		<!-- 忘記密碼後email輸入錯誤  -->
		<c:if test="${forgeterror==0}">
			<script>
				alert("email輸入錯誤");
			</script>
		</c:if>
		<!-- 忘記密碼後帳號輸入錯誤-->
		<c:if test="${forgeterror==1}">
			<script>
				alert("帳號輸入錯誤");
			</script>
		</c:if>
		...
		<section id="ms-banner-section" class="ms-contact-bnr">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h1 class="text-white ms-font-weight-900">忘記密碼	</h1>
						<div class="ms-seperator my-4"></div>
						<h5 class="pt-1 font-weight-normal text-white">
							<a class="text-white" href="/MOBEVA/">首頁</a> / 忘記密碼
						</h5>
					</div>
				</div>
			</div>
		...
		<section id="ms-abt-section" class="ms-sec-padding">
			<div>
				...
				<div align="center">
					<form method="POST" action="checkedacc_email">
						<table style="margin: auto; border: 1px;">
							<tr>
								<td><h3>請輸入帳號:</h3></td>
								<td><h3><input type="text" name="account" required></h3></td>
							</tr>
							<tr>
								<td><h3>請輸入email:</h3></td>
								<td><h3><input type="email" name="email" required></h3></td>
							</tr>
							<tr>
